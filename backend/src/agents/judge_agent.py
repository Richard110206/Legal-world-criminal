...
class JudgeAgent(BaseAgent):
    ...
    async def _on_indictment_filed(self, payload: dict) -> None:
        ...
        logger.info(f"[法官 {self.name}] 受理公诉案件: {case_id}")
    ...
    async def _on_defense_opinion_filed(self, payload: dict) -> None:
        ...
        if case_id not in self.case_queue:
            return
        logger.info(f"[法官 {self.name}] 收到辩护词，案件 {case_id} 控辩双方就绪")
        await self.schedule_trial(case_id, "first_instance")

    async def _on_criminal_appeal_filed(self, payload: dict) -> None:
        court_level = payload.get("court_level", "intermediate")
        if court_level != self.court_level:
            return
        case_id = payload.get("case_id", "")
        logger.info(f"[法官 {self.name}] 受理刑事上诉案件: {case_id}")
        if self.storage and self.config_path:
            self.storage.append_to_queue(self.config_path, "case_queue", case_id)
