...
class ReceptionistAgent(BaseAgent):
    ...
    def __init__(
        ...
        self._queued_client_sofas: dict[str, dict[str, Any]] = {}
        self._queued_client_wait_spots: dict[str, dict[str, Any]] = {}
        ...
        # storage 由 super().__init__ 保管，此之后才能读 roster 解析律所中文名
        self._firm_display_name = self._resolve_firm_display_name()
        self.name = f"前台接待·{self._firm_display_name}"
    ...
    def _resolve_firm_display_name(self) -> str:
        """Read the Chinese firm name from the roster; fall back to firm_id."""
        storage = getattr(self, "storage", None)
        base_dir = getattr(storage, "base_dir", None)
        if not base_dir:
            return self.firm_id
        roster_path = base_dir / "law_firms" / self.firm_id / "lawyer_roster.yaml"
        try:
            roster = storage.load_yaml(roster_path)
            firm_name = str(roster.get("firm_name", "") or "").strip()
            return firm_name or self.firm_id
        except Exception:
            return self.firm_id
    ...
    async def _llm_match(self, payload: dict) -> Optional[str]:
        ...
        # 前台阶段只注入"案情概览"（罪名/被告人/强制措施），不注入完整 case_background，
        # 让当事人做概括性来意陈述；具体案情留到委托洽谈(LC)阶段由律师面谈展开。
        case_summary = self._build_client_case_summary(client_config, client_agent)
    ...
    def _build_rule_match_client_need(self, party_role: str) -> str:
        if str(party_role or "").strip().lower() == "defendant":
            return (
                "你好，我的家人因为涉嫌刑事犯罪正在接受调查，"
                "我们想委托辩护律师，麻烦帮我安排合适的律师。"
            )
        return "你好，我想咨询一下这个案件该怎么处理，麻烦帮我安排合适的律师。"

    def _build_client_case_summary(self, client_config: dict, client_agent) -> str:
        """从前台可用的数据构建"案情概览"，供当事人在前台做概括性陈述。

        只提取罪名、被告人、强制措施等概括信息，不注入完整 case_background，
        避免当事人提前复述具体案情细节。
        """
        dataset_path = str(client_config.get("dataset_path", "") or "").strip()
        if not dataset_path:
            return ""
        try:
            ...
            case = data_loader.resolve_case_for_config(
                client_config,
                fallback_name=getattr(client_agent, "name", ""),
            )
            if not case:
                # 按 case_id 直接索引，绕开 profile 名称匹配（刑事数据集 party_info 只有被告）
                case = data_loader.get_case(int(str(client_config.get("case_id", "0") or "0") or "0"))
            info = (case or {}).get("extracted_info", {}) or {}
            if not isinstance(info, dict):
                info = {}
        except Exception as exc:
            logger.warning("[FrontDesk %s] failed to build client case summary: %s", self.firm_id, exc)
            return ""
        ...
        if not isinstance(measures, dict):
            measures = {}
        custody_parts = []
        if str(measures.get("detention", "") or "").strip():
            custody_parts.append(str(measures["detention"]))
        if str(measures.get("arrest", "") or "").strip():
            custody_parts.append(str(measures["arrest"]))
        if str(measures.get("custody_status", "") or "").strip():
            custody_parts.append(str(measures["custody_status"]))
    ...
    async def _publish_rule_match_recommendation(
        ...
        roster = self._load_roster()
        lawyer_name = self._lookup_lawyer_name(lawyer_id, roster) or "本所律师"
    ...
    async def _publish_final_assignment_correction(
        ...
        roster = self._load_roster()
        final_lawyer_name = self._lookup_lawyer_name(final_lawyer_id, roster) or "本所律师"
        text = (
            f"刚才推荐的律师当前无法立即接待，"
            f"最终为您安排{final_lawyer_name}律师接续办理。"
        )
        if hasattr(self.map_engine, "broadcast_dialogue"):
            await self.map_engine.broadcast_dialogue(
                case_id,
    ...
    def _format_roster_for_prompt(
        ...
        """Render roster data for the reception prompt（只含中文名，不暴露内部编号）。"""
        lawyers = roster.get("lawyers", [])
        ...
        for index, lawyer in enumerate(lawyers, start=1):
            ...
            lines.append(
                f"- {index}. {name or '律师'} | 专长：{specialty or '综合'} | 资历：{seniority} | 状态：{status}{case_hint}"
            )
