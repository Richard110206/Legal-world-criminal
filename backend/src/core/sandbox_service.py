...
class SandboxService:
    ...
    def _resolve_initial_client_profile(self, *, config: dict[str, Any], dataset_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            profile = loader.extract_plaintiff_profile(case)
            if str(profile.get("name", "") or "").strip():
                return profile
            # 刑事数据集 party_info 只有 defendant：原告位=委托方（被告人家属），
            # 用被告人家属视角兜底命名，避免前端落到 case_N_plaintiff 系统名
            case_category = str(
                (case.get("extracted_info", {}) or {}).get("case_category")
                or (case.get("extracted_info", {}) or {}).get("case_type")
                or ""
            ).strip().lower()
            if case_category == "criminal" or str(config.get("case_category", "") or "").strip().lower() == "criminal":
                defendant_name = str(
                    ((case.get("extracted_info", {}) or {}).get("party_info", {}) or {})
                    .get("defendant", {})
                    .get("name", "")
                    or ""
                ).strip()
                fallback_name = f"{defendant_name}家属委托人" if defendant_name else "嫌疑人家属委托人"
                profile["name"] = fallback_name
                profile["type"] = profile.get("type", "") or "自然人"
                profile["party_type"] = profile.get("party_type", "") or "自然人"
            return profile
        except Exception:
            return {}
