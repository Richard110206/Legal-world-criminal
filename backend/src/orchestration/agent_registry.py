...
class AgentRegistry:
    ...
    def _discover_prosecutors(self) -> None:
        """Scan sandbox_data/court_system/procuratorate*/prosecutors/*/config.yaml (criminal)."""
        ...
        for procuratorate_dir in sorted(court_dir.iterdir()):
            if not procuratorate_dir.is_dir():
                continue
            if "procurator" not in procuratorate_dir.name:
                continue

            prosecutors_dir = procuratorate_dir / "prosecutors"
            if not prosecutors_dir.exists():
                continue

            for prosecutor_dir in sorted(prosecutors_dir.iterdir()):
                config_file = prosecutor_dir / "config.yaml"
                if not config_file.exists():
                    continue

                config = self.storage.load_agent_config(str(prosecutor_dir))
                profile = config.get("profile", {})
                agent_id = prosecutor_dir.name

                agent = ProsecutorAgent(
                    agent_id=agent_id,
                    name=profile.get("name", agent_id),
                    procuratorate=profile.get("organization", "人民检察院"),
                    config_path=str(prosecutor_dir),
                    storage=self.storage,
                )
                self._agents[agent_id] = agent
                logger.info(
                    f"[Registry] Discovered prosecutor: {agent.name} ({agent_id}) @ {agent.procuratorate}"
                )
