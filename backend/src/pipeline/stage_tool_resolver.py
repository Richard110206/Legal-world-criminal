"""Manifest-driven stage tool resolution and runtime injection — 纯刑事。"""
...
ALLOWED_AGENT_TYPES: tuple[str, ...] = (
    "lawyer", "client", "judge", "receptionist",
    "prosecutor",  # 刑事: 检察官
)
ALLOWED_STAGE_ROLE_NAMES: tuple[str, ...] = (
    "client", "lawyer", "plaintiff", "defendant",
    "judge",
    # 刑事
    "prosecutor",         # 检察官/公诉人
    "investigator",       # 公安侦查员
    "defense_lawyer",     # 辩护律师（刑事庭审场景专用）
    "appellant",          # 刑事上诉人（被告人）
    "appellant_lawyer",   # 上诉人辩护律师
)
...
def resolve_agent_type(agent: Any) -> str:
    ...
    type_map = {
        ...
        "ProsecutorAgent": "prosecutor",        # 刑事
        "InvestigatorAgent": "client",          # 刑事: 复用 client type
    }
...
def infer_stage_role_name(stage_code: str, agent: Any) -> str:
    ...
    if normalized_stage_code == "CRA":
        if agent_type == "judge":
            return "judge"
        if agent_type == "prosecutor":
            return "prosecutor"
        if agent_type == "client" and party_role in {"appellant", "defendant"}:
            return "appellant"
        if agent_type == "lawyer" and court_role in {"appellant", "defendant"}:
            return "appellant_lawyer"
        if agent_type == "lawyer":
            return "defense_lawyer"
        return ""
...
def describe_stage_tool_matrix() -> dict[str, dict[str, list[str]]]:
    ...
    for stage_code in REAL_STAGE_CODES:
        ...
        for role_name in stage_roles:
            ...
            if role_name in {"client", "plaintiff", "defendant", "appellant"}:
                default_agent_type = "client"
            elif role_name == "judge":
                default_agent_type = "judge"
            elif role_name == "prosecutor":
                default_agent_type = "prosecutor"
