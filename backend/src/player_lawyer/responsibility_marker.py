"""Helpers for marking player-lawyer responsibility dialogue — 纯刑事。"""
...
RESPONSIBILITY_ROLES_BY_STAGE = {
    ...
    "INV": {"lawyer", "defendant_lawyer"},
    "PR": {"lawyer", "defendant_lawyer"},
    "DS": {"lawyer", "defendant_lawyer"},
    "CR": {"defense_lawyer", "defendant_lawyer"},
    "CRA": {"defense_lawyer", "defendant_lawyer"},
}
...
def build_player_responsibility_marker(
    *,
    ...
    if normalized_role not in RESPONSIBILITY_ROLES_BY_STAGE.get(normalized_stage, set()):
        return None
    if normalized_stage == "LC" and _looks_like_consultation_auto_opening(content):
        return None
