...
``SIMLAW_PLAYER_LAWYER_MODE=defendant`` is set (玩家扮演辩护律师).
"""
...
from .agent import is_player_defendant_mode
...
def _player_lawyer_status_payload(request: Request) -> dict[str, Any]:
    ...
    return {
        "player_mode": "defendant" if is_player_defendant_mode() else "off",
        "enabled": is_player_defendant_mode(),
    }
