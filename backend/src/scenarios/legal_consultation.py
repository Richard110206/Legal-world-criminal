        ...
        if party_role == "defendant":
            if role == "client":
                if self.dialog_history:
                    return "我明白了，会按您说的配合辩护工作。谢谢律师。【咨询结束】"
                return "我的家人因涉嫌犯罪被采取强制措施，想咨询委托辩护的事。"
            return (
                "我先按家属陈述和现有材料为您梳理辩护思路：核实涉嫌罪名与强制措施，"
                "说明侦查阶段律师工作与取保候审可能，下一步接受委托进入会见。"
            )
        if role == "client":
            if self.dialog_history:
                return "我明白了，会按您说的准备材料。谢谢律师。【咨询结束】"
            return "我想咨询一下刑事案件该怎么委托辩护、需要准备哪些材料。"
        return (
            "我先按现有材料为您梳理辩护思路：核对涉嫌罪名、强制措施、办案阶段，"
            "说明委托辩护的流程与侦查阶段律师能做的具体工作。"
        )
