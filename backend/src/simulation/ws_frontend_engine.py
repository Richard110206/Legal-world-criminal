...
class WebSocketFrontendEngine(TownAvatarInterface):
    ...
    def __init__(
        ...
        # 外部注入：玩家输入未决检查（返回 True 时对话 gate 挂起不推进）
        self._player_pending_check: Any = None

        # ── Agent 状态追踪（用于新客户端同步） ──
        # key = agent_id, value = 当前状态快照
        self._agent_states: dict[str, dict[str, Any]] = {}

    def set_player_pending_check(self, check: Any) -> None:
        """Register a callable returning True while a player input request is pending."""
        self._player_pending_check = check

    def _player_input_pending(self) -> bool:
        check = self._player_pending_check
    ...
    async def _wait_for_dialogue_continue(
        self,
        case_id: str,
        speaker_name: str,
        turn: int,
    ) -> None:
        """Fallback turn pacing when no gate-capable client is connected.

        The gate-capable path is handled earlier in broadcast_dialogue via
        _buffer_dialogue_until_continue; reaching here means plain broadcast
        mode, so no additional wait is required.
        """
        return None
    ...
    async def _buffer_dialogue_until_continue(
        ...
        try:
            ...
            # 玩家模式（player_v2 客户端在场）必须点击推进，不自动放行；
            # 仅纯自动观战（auto 无玩家客户端）保留超时自动继续兜底。
            has_player_client = FRONTEND_MODE_PLAYER_V2 in set(self._client_frontend_modes.values())
            auto_remaining = (
                self._dialogue_gate_auto_timeout_seconds()
                if self._turn_mode == TURN_MODE_AUTO and not has_player_client
                else None
            )
            while True:
                if self._paused:
                    await self._resumed_event.wait()
                    continue
                # 玩家输入未决：挂起推进，直到玩家提交（或请求被取消/过期）
                if self._player_input_pending():
